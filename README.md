# Results from COBIND project 2022

This is repository that stores the results from COBIND 2022 analysis.

To access the results page go to [https://github.com/ievarau/COBIND_2022](https://github.com/ievarau/COBIND_2022).

## COBIND tool ##

COBIND is available as a tool [**here**](https://bitbucket.org/CBGR/cobind_tool/src/master/).

## COBIND pipeline for manuscript results ##

The version of COBIND pipeline that was used to analyse data for the manuscript is available [**here**](https://bitbucket.org/CBGR/pipeline_cobind/src/master/).

## Contact ##

In case you have any questions related to this pipeline, please contact the following persons:
|                                                           |                                 |
| ----------------------------------------------------------|---------------------------------|
| [Ieva Rauluseviciute](https://bitbucket.org/ievara/)      | ieva.rauluseviciute@ncmm.uio.no |
| [Anthony Mathelier](https://mathelierlab.com/)            | anthony.mathelier@ncmm.uio.no   |
